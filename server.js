...
app.post('/delete', (req, res) => {
  ...
  for (let i = 0; i < basket.length; i++) {
    if (basket[i].name === delObj.product.name) {
      sumOfProducts = sumOfProducts - basket[i].price; console.log(`sum=${sumOfProducts}`);
      basket.splice(i, 1);
    }
  }
...
app.post("/buy", (req, res) => {//result of transaction
  const { balance } = req.body;
  if (balance >= sumOfProducts && basket != []) {
    basket = []; sumOfProducts = 0;//full reset main values for next cycle
    res.json({ success: true, sum: sumOfProducts })
  }
